...
class RLAgent(Agent[S, A]):
    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            idx = self.rng.int(0, len(actions) - 1)
            return actions[idx]
        else:
                # TODO: return the action with the maximum q-value as calculated by "compute_q" above
             # Return the action with maximum Q (ties -> first in actions list)
            best_action = actions[0]
            best_q = self.compute_q(env, observation, best_action)
            for a in actions[1:]:
                q = self.compute_q(env, observation, a)
                if q > best_q:
                    best_q = q
                    best_action = a
            return best_action
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        if next_action is None:
            target = reward  # terminal -> Q(s',a') = 0
        else:
            target = reward + self.discount_factor * self.Q[next_state][next_action]
        self.Q[state][action] = cur_q + self.learning_rate * (target - cur_q)
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        # Utility = max_a Q(state, a). If there are no available actions, return 0.
        actions = env.actions()
        if not actions:
            return 0.0
        best = None
        for a in actions:
            q = self.compute_q(env, state, a)
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        if done:
            ...
            target = reward + self.discount_factor * self.compute_utility(env, next_state)
        self.Q[state][action] = cur_q + self.learning_rate * (target - cur_q)
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        # Dot-product between weights[action] and features
        weights_for_action = self.weights[action]  # action is used as-is (same type as stored keys)
        total = 0.0
        for fname, fval in features.items():
            w = weights_for_action.get(fname, 0.0)
            total += w * fval
        return total
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # prediction = Q(s,a) = w_a . x(s)
        # target = r + gamma * max_a' Q(s', a')   (or r if done)
        features = self.feature_extractor.extract_features(env, state)
        ...
        for fname, fval in features.items():
            # gradient step: w_i += alpha * td_error * feature_i
            w[fname] = w.get(fname, 0.0) + self.learning_rate * td_error * fval
        # store back (dictionary mutates in place, but we keep semantics)
